...
**Version 1.1 · September 2026 · Applies to every paid Amanorsac app, plugin and plugin bundle.**

This is the one licensing system for every product sold on amanorsac.studio.
It is live and proven end to end: SecondOut, AETHER, AMB Analog and Align
Pro all activate against it today. A build that follows this document will
activate for every buyer. A build that departs from it will not be accepted.

The words **MUST** and **MUST NOT** mark requirements. Everything else is
explanation.

**A product may also ship with no licensing at all.** AFD Gate does: it
never contacts a server, works with no network connection and asks nobody
who they are. If that is right for your product, say so at the start — the
studio marks the catalog entry and the store page accordingly, and none of
this document applies to you. What is not acceptable is a half-built
licence path, or one that reports success without verifying a proof.

**Nothing here is about updating the product.** Amanorsac Hub installs,
updates and opens every product. Do not build a version check, an
updater, or a "new version available" notice.

**Version 1.1 changes.** The reference build is SecondOut 1.3.2. The
storage folder table no longer carries the retired Aquarii Audio path.
The app id and key prefix are assigned by the studio, not derived by the
developer (§3). Delivery (§10) now matches what the store catalog needs.
...
- Block 1 comes from the app id (`secondout` → `SECO`, `ambanalog` → `AMBA`).
  **The studio assigns the app id and tells you the prefix.** It is not
  always the obvious contraction of the product name: the prefix is printed
  on a customer's key, and some contractions are not printable. Ask for it
...
every licensed product in the store ships it and activates with it.
...
SecondOut 1.3.2's `LicenseClient.h` and `LicenseCrypto.h` implement §4 to §7
completely in plain JUCE/C++ with no other dependencies. Ask the studio for
both files. For a new product, change only these:

| Constant | Your product |
|----------|--------------|
| Storage folder | `Amanorsac Studio\<product>` — never `Aquarii Audio`, which is retired. If an older build of your product wrote there, migrate on first run and stop using it |
| DPAPI entropy string | `"<product>/license/v1"` |
| DPAPI description | `L"<product>"` |
| Default base URL | `https://amanorsac.studio` — unchanged (R1) |
| `kLicenseSigningKey` | the §5 bytes — unchanged (R2) |
...
1. **The facts for the catalog entry:** the app id (confirmed with the
   studio, §3), the display name, the version string, the platforms, the
   plugin formats, and the price.
2. **The installers**, one per platform, signed and notarised, with their
   **exact filenames and byte sizes** — the store looks a file up by name,
   and a file uploaded under a name the catalog does not expect is
   invisible to the site.
3. **The completed §9 report**, A1 to A12, with the key you used.
4. For a **bundle**, the list of plugins it contains, and confirmation of
   A10 specifically.
5. For an **unlicensed** product, a written statement that it ships with
   no licensing and makes no network requests at all, in place of items 3
   and 4.

The studio then adds one catalog entry and the price, uploads the
installer, and the product is on sale. The license server needs no change
for a new product.
...
These are not hypotheticals. SecondOut 1.3.0 shipped with two faults, and
either alone blocked every sale:
...
Both were fixed in 1.3.1 and the product now ships as 1.3.2, which
activates correctly against the live server.

A third fault is worth knowing because it was reported as a licensing bug
and was not one: a customer removed both devices in *My Apps* and the app
kept opening without asking for a key. The cause was on the server — a
removed device could not be re-activated because of a unique constraint,
and the fix was a database change, not a client one. If activation
behaves in a way this document does not explain, say so before working
around it. The answer may not be in your build.
